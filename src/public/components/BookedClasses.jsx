...
const BookedClasses = ({attendances, lessons, types, teachers, locations}) => {
  return (
    <div>
      ...
      {attendances.map((attendance, i) =>
        <div key={"attendance" + i} className="row">
          ...
          <div className="col s12 m1 l1">
            <button className="btn booking-btn waves-effect waves-light red accent-1" onClick={() => { this.unbookLesson((Auth.getUser().id), ((lessons).find((lesson) => {return (lesson._id === attendance.lesson_id)})._id)) }}>
              Cancel
            </button>
          </div>
        </div>
